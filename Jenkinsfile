pipeline {
  ...
  stages {
    ...
    stage('composer') {
      steps {
        sh "cd docker4php ; source \$PWD/.pname && export CMD='cd / ; sudo rm -rf /var/www/html ; sudo ln -s /opt/var/www/html /var/www/html  ; cd /var/www/html ;rm -rf web/core ;  sudo chmod -R 777 /opt/var/www/html  ; composer install' &&  sh -c 'make fpmi '"
      }
    }
